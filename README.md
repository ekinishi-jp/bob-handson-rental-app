...
| モード | 説明 |
| --- | --- |
| Planモード | 実装前の計画・設計・戦略立案に特化したモードです。Markdownファイル(.md)のみ編集可能で、技術仕様書、アーキテクチャ設計、要件定義書の作成に使用します。 |
| Codeモード | コードの作成、修正、リファクタリングを実行するモードです。新機能実装、バグ修正、テストコード作成に使用します。 |
| Advancedモード | 全てのファイル編集とコマンド実行が可能なモードです。Codeモードと同等の機能を提供しつつ、より複雑な実装タスクや大規模な変更、システム全体に影響する高度なリファクタリングに使用します。 |
| Askモード | 説明、ドキュメント、技術的な質問への回答に特化したモードです。コード変更は行わず、分析と情報提供のみを行い、Mermaid図を使った視覚的な説明も可能です。 |
| Orchestratorモード | 複数ステップにわたる複雑なプロジェクトの調整に特化したモードです。大規模タスクをサブタスクに分解してワークフローを管理し、異なる専門分野や複数のドメインを横断する開発に使用します。 |
...
- **追加機能の要件定義・実装**: 内見予約機能だけを対象にします。問い合わせ詳細API `GET /api/inquiries/{inquiry_id}` は変更しません。
- **コードレビュー・セキュリティレビュー**: 追加実装した内見予約機能の品質確認と、既存の問い合わせ詳細APIに意図的に残している認可不備（IDOR）の確認・修正を行います。
...
BackendとFrontendの起動コマンドを実行し、ブラウザで確認できるURLを教えてください。
```
...
既存アプリに追加する「内見予約」機能の要件定義をリクエストします。
...
2. **Bobに追加機能設計書を作成するようにリクエストを出します**
   ```text
   物件詳細画面から内見希望日時を選び、予約を登録・確認できる機能を追加したい。
   ```
...
3. **Bobから仕様を詰めるための質問が来る場合は適宜答えていきます**
   ...
   例：
   ```text
   物件詳細画面に「内見予約」セクションを追加し、希望日時、氏名、メールアドレス、電話番号を入力して予約できるようにする。
   予約可能枠はバックエンドAPIから取得し、予約済みの枠は選択できないようにする。
...
実装対象は内見予約機能です。セクション間の独立性を保つため、問い合わせ詳細API `GET /api/inquiries/{inquiry_id}` の認可処理はここでは修正しません。
...
5. **実装が完了したら、動作確認を行います**
   ...
   期待する追加実装の例：
   - `GET /api/properties/{property_id}/viewing-slots`
   - `POST /api/viewing-reservations`
   - 物件詳細画面の内見予約フォーム
   - 予約完了表示
   - 予約APIの単体テスト
...
`/review`を使用して、追加実装した内見予約機能の品質と、既存APIに意図的に含めている認可不備を確認します。
...
3. **Bob Findingsで解析結果を確認します**
   ...
   追加実装した内見予約機能の品質、テスト不足、実装ミスに加えて、問い合わせ詳細APIの認可チェック不足が検出されることを確認します。
...
5. **Bobに修正を依頼します**
   ...
   ```text
   問い合わせ詳細APIで、ログイン中ユーザーの所有データのみ返すように修正してください。
   他ユーザーの問い合わせIDを指定した場合は404を返してください。
   ```
...
問い合わせやお気に入りなど、ハンズオン中に登録したデータを削除する場合は、`rental.db` を削除します。
...
以下は講師向けの模範解答例です。参加者に最初から見せる必要がない場合は、配布対象から除外してください。

- [`instructor/hands_on_feature_model_answer.md`](./instructor/hands_on_feature_model_answer.md)
- [`instructor/security_review_model_answer.md`](./instructor/security_review_model_answer.md)
